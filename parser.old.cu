...
Node newNode(string tp, string val, IntArr children) {
  return new Node(tp, val, children);
}

string getType (Node node) { return node.tp; }
string getVal (Node node) { return node.val; }
IntArr getChildren (Node node) { return node.children; }
...
import cobre.record(string, string, int) {
  type `` as Token;
  string get0(Token) as TkType;
  string get1(Token) as TkVal;
  int get2(Token) as TkLine;
}
...
void check (Token tk, string tp) {
  if (TkType(tk) == tp) {}
    ...
    string msg = "Expected " + tp + " but found " + TkType(tk);
    print(msg + ", at line " + itos(TkLine(tk)));
    quit(1);
  }
}
